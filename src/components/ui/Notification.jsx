...
const Notification = () => {
    ...
    const updateSubscriptionOnServer = async (isSubscribed, subscription) => {
        ...
        axios
            .put(
                `${process.env.REACT_APP_BACKEND_URL}/api/v1/users/${userData.user.name}/subscription`,
                subscription,
                config
            )
        ...
        try {
            await axios.put(
                `${process.env.REACT_APP_BACKEND_URL}/api/v1/users/${userData.user.name}`,
                { isSubscribed },
                config
            );
        } catch (err) {
    ...
    const handleSubscribe = async () => {
        const resPublicVapidKey = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/subscribe`, config);
